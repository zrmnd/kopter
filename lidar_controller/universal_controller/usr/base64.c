...
uint32_t base64_encode(const uint8_t* bytes_to_encode, uint32_t in_len,
    uint8_t* encoded_buffer, uint32_t out_len_) {
  uint32_t out_len = out_len_;
  int i = 0;
  int j = 0;
...
uint32_t base64_decode(const uint8_t* encoded_string, uint32_t in_len,
    uint8_t* decoded_buffer, uint32_t out_len_) {
  uint32_t out_len = out_len_;
  size_t i = 0;
  size_t j = 0;
  int in_ = 0;
