...
void USART1_IRQHandler(void) {
	...
	//	uint8_t t = USART1->DR;
	//	sendUart3(&t, 1);
		/*uart1_buff[receiveIndex_uart1++] = t;
		if (receiveIndex_uart1 == UART_BUF_SIZE)
			receiveIndex_uart1 = 0;
		if (receiveIndex_uart1 == uart1_buf_index) // обработка ситуации, когда получили новое быстрее, чем обработали старое
			uart1_flagORE = 1;*/
	//}
	//USART1->SR &= ~ (USART_SR_IDLE | USART_SR_ORE | USART_SR_RXNE | USART_SR_TXE);
	...
	if (USART1->SR & USART_SR_RXNE) {
		...
		if (t == '\r' && receiveIndex < UART_BUF_SIZE) { // msg received, parse; 
			uart2_buff[receiveIndex] = 0;
			sendUart3(&uart2_buff[1], receiveIndex-1);
		}
...
void USART3_IRQHandler(void) {
	if (USART3->SR & USART_SR_RXNE) {
		uint8_t t = USART3->DR;
		sendUart1(&t, 1);
		/*uart3_buff[receiveIndex_uart3++] = t;
		if (receiveIndex_uart3 == UART_BUF_SIZE)
			receiveIndex_uart3 = 0;
		if (receiveIndex_uart3 == uart3_buf_index) // обработка ситуации, когда получили новое быстрее, чем обработали старое
			uart3_flagORE = 1;*/
	}
	USART3->SR &= ~ (USART_SR_IDLE | USART_SR_ORE | USART_SR_RXNE | USART_SR_TXE);
}
//------------------------------------------------------------------------------
