...
def srv_interpreter_thread(cmdAgent, flagStopThread, flagDropConnection): #receive comands over TCP-IP, handle them and send answers
	...
	srv_interpreter.listen(10)
	srv_interpreter.setblocking(0)
	cmdAgent.setReady()
	#flagDropConnection = False
	while flagStopThread.value == 0:
		# получает клиентов:
		try:
			client, addr = srv_interpreter.accept()
			print "Accepted interpreter client...\n"	
			client.setblocking(0) # снимаем блокировку и тут тоже		
			flagDropConnection.value = 0
			while (flagDropConnection.value == 0) and (flagStopThread.value == 0):
			...
			try:
				...
				pass
			print "Interpreter connection broken or no connections!" 
	...
	if __name__ == '__main__':
		...
		while True:
			...
			if interpreterComandAgent.isReady() == False:
				interpreterComandAgent.addAnswer("Readed!")
				time.sleep(2)
